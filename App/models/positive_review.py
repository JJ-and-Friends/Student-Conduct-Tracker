...
positive_review_staff_upvoters = db.Table(
    'positive_review_staff_upvoters',
    db.Column('reviewID', db.Integer, db.ForeignKey('review.ID')),
    db.Column('staffID', db.String(10), db.ForeignKey('staff.ID')),
)

positive_review_staff_downvoters = db.Table(
    'positive_review_staff_downvoters',
    db.Column('reviewID', db.Integer, db.ForeignKey('review.ID')),
    db.Column('staffID', db.String(10), db.ForeignKey('staff.ID')),
)


class Positive_Review(db.Model,review):
  __tablename__ = 'positive_review'
...
#Direct access for staff instance to the Staffdownvoters on a positive review
  staffUpvoters = db.relationship(
      'Staff',
      secondary= positive_review_staff_upvoters,
      backref=db.backref(
          'reviews_upvoted',
 ...
 #Direct access for staff instance to the Staffdownvoters on a positive review
  staffDownvoters = db.relationship(
      'Staff',
      secondary= positive_review_staff_downvoters,
      backref=db.backref(
          'reviews_downvoted',
          lazy='joined'))  #for staff who have voted on the review
...
#allows the comment to be edited if the staff member is the creator of the review, returns none if not
