...
negative_review_staff_upvoters = db.Table(
    'negative_review_staff_upvoters',
    db.Column('reviewID', db.Integer, db.ForeignKey('review.ID')),
    db.Column('staffID', db.String(10), db.ForeignKey('staff.ID')),
)

negative_review_staff_downvoters = db.Table(
    'negative_review_staff_downvoters',
    db.Column('reviewID', db.Integer, db.ForeignKey('review.ID')),
    db.Column('staffID', db.String(10), db.ForeignKey('staff.ID')),
)


class Negative_Review(db.Model):
  __tablename__ = 'negative_review'
...
#Direct access for staff instance to the Staffdownvoters on a positive review
  staffUpvoters = db.relationship(
      'Staff',
      secondary= negative_review_staff_upvoters,
      backref=db.backref(
          'reviews_upvoted',
 ...
 #Direct access for staff instance to the Staffdownvoters on a positive review
  staffDownvoters = db.relationship(
      'Staff',
      secondary= negative_review_staff_downvoters,
      backref=db.backref(
          'reviews_downvoted',
          lazy='joined'))  #for staff who have voted on the review
